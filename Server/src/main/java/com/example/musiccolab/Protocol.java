        ...
        if (action == createLobby && player != null) {
            ...
            if (!Server.lobbyList.contains(lobbyName)) {
                ...
                if (action == joinLobby) {
                    ...
                    if (checkResponse)
                        Main.logr.log(Level.INFO, "CLIENT " + playerAddress.toString() + " JOINED LOBBY " + currentLobby.getLobby_id());
                    action = 21;
                    ...
                    if (checkResponse) {
                        ...
                        if (player.isAdmin()) {
                            player.disableAdmin();
                            responseAction = becameAdmin + 10;
                            sendResponseToClient(messageCharset, clientChannel, "You are no more admin.");
                            if (currentLobby.getAdmin() != null) {
                                responseAction = becameAdmin;
                                sendResponseToClient(messageCharset, currentLobby.getAdmin().getPlayerChannel(), "You are now admin.");
                            }
                        }
